...
public class EmailService {
	...
	NumberFormat decimalFormatter = DecimalFormat.getInstance();
	...
	public void sendClientEmail(final User user, final Transaction transaction) {
		...
		Runnable r  = new Runnable(){
			...
			@Override
			public void run() {
				String subject = "Transaction for card no " + transaction.getCreditCardNo() + " for " + decimalFormatter.format(transaction.getAmount()) +" at " + transaction.getMerchant() + " needs approval.";
